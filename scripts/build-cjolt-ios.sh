#!/bin/bash
# Build script for cjolt framework on iOS
# Usage: ./build-cjolt-ios.sh [architecture] [build_type]
# Example: ./build-cjolt-ios.sh arm64 Release
# Example: ./build-cjolt-ios.sh x86_64 Debug (for simulator)
...
BUILD_DIR="$JOLT_DIR/build-xcode-ios"
...
echo "=========================================="
echo "Building cjolt framework for iOS"
...
DEST_DIR="$JOLT_DIR/libs/ios/$(echo "$BUILD_TYPE" | tr '[:upper:]' '[:lower:]')"
mkdir -p "$DEST_DIR"

# Copy framework to destination
echo "Copying framework to $DEST_DIR..."
FRAMEWORK_PATH="$BUILD_DIR/$BUILD_TYPE-$SDK/cjolt.framework"
DEST_FRAMEWORK="$DEST_DIR/cjolt.framework"
...
if [ -d "$FRAMEWORK_PATH" ]; then
    ...
    find "$BUILD_DIR" -name "cjolt*" -type f 2>/dev/null || echo "No cjolt files found"
    exit 1
fi
...
if [ -d "$DEST_FRAMEWORK" ]; then
    echo "✓ Successfully built cjolt framework"
    ...
    if [ -f "$DEST_FRAMEWORK/cjolt" ]; then
        file "$DEST_FRAMEWORK/cjolt"
    else
        echo "Warning: Framework binary not found"
    fi
else
    echo "✗ Failed to build cjolt framework"
    exit 1
fi
