...
| `UnitTests/Math/MathTests.cpp` | `Tests_Math.cs` | 🚫 | 12 | Basic Vec3/Quat covered; remaining MathTests are internal SIMD/trig helpers not exposed by C bindings |
| `UnitTests/Math/VectorTests.cpp` | `Tests_Vector2Matrix.cs` | ✅ | 26 | Vector_2 (15 tests): construct/copy/+/-/*/÷/Dot/LengthSq/Length/Normalized/==; Matrix_2_2 (11 tests) covered by same file — see MatrixTests row |
| `UnitTests/Math/MatrixTests.cpp` | `Tests_Vector2Matrix.cs` | ✅ | — | Matrix_2_2 (11 tests): SZero/SIdentity/construct/SetRow/GetRow/==/*/Vec multiply; shared with VectorTests.cpp coverage in Tests_Vector2Matrix.cs |
...
| Native Test File | C# Test File | Status | Tests | Notes |
|---|---|---|---|---|
| `UnitTests/Physics/PhysicsTests.cpp` | `Tests_Physics.cs` | ✅ | 10 | Basic simulation: add/step/position, AddForce, UserData, collision detection, kinematic motion, OptimizeBroadPhase |
...
| `UnitTests/Physics/ObjectLayerPairFilterTableTests.cpp` | `Tests_ObjectLayers.cs` | ✅ | 9 | ObjectLayerPairFilterTable construct, EnableCollision, ShouldCollide, GetNumObjectLayers, ObjVsBroadPhase mapping |
...
| `UnitTests/Physics/BroadPhaseTests.cpp` | `Tests_PhysicsQuery.cs` + `Tests_CastResult.cs` | 🚫 | 29 | GetBroadPhaseQuery / GetNarrowPhaseQuery, GetBounds after body add, WereBodiesInContact, BroadPhaseCastResult/RayCastResult result struct covered; internal broadphase tree algorithms and cast execution use template collectors not exposed by C bindings |
...
| `Tests_CharacterVirtual.cs` | 23 | CharacterVirtual construction; GetPosition/SetPosition; GetRotation; GetLinearVelocity/SetLinearVelocity; GetMass/SetMass; GetMaxStrength/SetMaxStrength; GetPenetrationRecoverySpeed; GetEnhancedInternalEdgeRemoval; GetMaxNumHits; GetGroundState (InAir); GetGroundBodyID (invalid); GetListener (null); GetUp/SetUp; GetCosMaxSlopeAngle/SetMaxSlopeAngle; Update no-crash; Update multi-step no-crash |
